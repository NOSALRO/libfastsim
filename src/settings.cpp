...
namespace fastsim {
    Settings::Settings(const std::string& config_file)
    {
        ...
        _map = std::shared_ptr<Map>(new Map(_get_a<std::string>(n_map, "name").c_str(),
            _get_a<float>(n_map, "size")));

        ptree& n_robot = pt.get_child("fastsim.robot");
        _robot = std::shared_ptr<Robot>(new Robot(_get_a<float>(n_robot, "diameter"),
