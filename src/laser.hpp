...
namespace fastsim {
    class Laser {
    public:
        Laser(float angle, float range, float gap_dist = 0.0f, float gap_angle = 0.0f) : _angle(angle), _range(range), _gap_dist(gap_dist), _gap_angle(gap_angle), _x(0), _y(0), _x_pixel(0), _y_pixel(0), _dist(-1) {}
        float update(const Posture& pos, const std::shared_ptr<Map>& map);
