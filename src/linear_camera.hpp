...
namespace fastsim {
    ...
    public:
        ...
        LinearCamera(float angular_range, int nb_pixels) : _angular_range(angular_range), _pixels(nb_pixels)
        {
            std::fill(_pixels.begin(), _pixels.end(), -1);
        }

        void update(const Posture& pos, const std::shared_ptr<Map>& map);
